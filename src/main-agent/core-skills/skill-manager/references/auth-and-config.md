...
A skill may need credentials (API keys, tokens) or non-secret configuration (endpoints, limits, defaults). This is the operating convention for how an Agent manages those files when installing an external skill or creating its own.

## File layout

Every skill keeps its own files inside its skill directory (`skills/<skill-name>/`):
...
Credentials do not appear in `SKILL.md`, in scripts, in messages, in notes, or in any other persisted or shared text. Scripts read them from `auth.json` at run time; they are never echoed into output, logs, or chat. When a human provides a credential, the Agent writes it into the skill's `auth.json` and references it only as a file.

## Creating or installing a skill

- Create or copy the skill directory at `skills/<skill-name>/`.
