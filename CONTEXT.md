...
**Frozen Activity**:
面向一个已关闭 Active Segment、已停止变化并可供认知器官消费的实际活动证据，其中每个事件都通过 Actor Reference 明确归属。它必须完整覆盖该段内的 Turn、后续 Input、工具活动、Effect 与 Delivery，而不只是最近一个 Turn；超过容量而拆分时，按序的整组材料仍须无遗漏覆盖该段。它不是自然语言摘要。
