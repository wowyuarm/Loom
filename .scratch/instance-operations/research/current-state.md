...
| Raft | Raft keeps bridge replay data and channel state under the Instance Root; the Host owns `connecting`, `connected`, `degraded` and `stopped` status. [raft.md](../../../docs/integrations/raft.md) | The documentation explicitly says no `loom status` client exists. A running Host must not be read as Raft being connected. |
