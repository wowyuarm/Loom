...
- [ ] 真失败：退避 1min/5min，第 3 次置 needs_human；needs_human 后每 24h 冷却重试一轮，成功静默清标留痕
- [ ] 配额类错误：park 到 reset/6h，不计 attempts
- [ ] abort：域行不变、重跑不计 attempts、无 cancelled/intervention_required 终态
- [ ] attention 的 needs_human 不影响其他 organ 入口
- [ ] 全部现有测试保持绿色（台账相关断言随本票改写为域行断言）
