...
Attention Maintainer 是 Harness 内置并版本化的 Cognitive Organ。它只维护 `attention.md`，不承担调度、长期记忆、Daily、Episode、thread 结构或行为层维护。
...
2. **Attention Maintainer Interface**：用真实临时 Workspace 与 Pi faux provider 运行一次维护，观察最终 Context、可用工具、结果与 `attention.md`；不以 prompt 字符串断言代替模型质量。
...
- `AttentionMaintainer` 使用独立 Pi session、完整 Stable Facts 和索引式首轮 Context。它只获得 Workspace `read` / `ls` / `grep`、受限 `read_recent_activity` 与一次性 `replace_attention`。
...
完成后的对照审视进一步校准了 prompt，而没有改变材料合同：

- 器官正式缩名为 **Attention Maintainer**；**Current Attention** 只表示被维护的 Workspace 材料。
- 参照 Orientation 明确材料归属：器官不是 Main Agent 或 Individual，读到的身份、关系、兴趣与私人工作不会变成它自己的生活。
- 参照 Xi now-maintainer 将判断收成一个问题：缺少这条觉知时，未来几天的表达、方向、关系靠近或沉默判断是否会自然不同。可从 Daily/Activity/thread 找回细节，不等于它应进入 attention。
- 不再笼统禁止提及 Harness；只禁止暴露触发本次维护的内部机制。Harness 若本就是 Individual 的真实工作对象，可以自然进入 attention。
- provider 或最终输出失败时逐字恢复原文件，不再规范化旧内容的首尾空白。
