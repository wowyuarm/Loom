...
cd "$repo"
find dist/test -name '*.test.js' | sort | xargs -P 2 -n 1 node "$repo/.scratch/archive/test-ablation/harness/collect-one.mjs" "$repo" "$work"
node "$repo/.scratch/archive/test-ablation/harness/merge-coverage.mjs" "$repo" "$work" "$out"
